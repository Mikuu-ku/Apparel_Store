...
if ($query !== '') {
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<img src="assets/images/' . $image . '" style="width: 40px; height: 40px; object-fit: cover; margin-right: 15px; border: 1px solid #eee;">';
            
            echo '<div style="display: flex; flex-direction: column;">';
            echo '<span style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: #111;">' . $name . '</span>';
            echo '<span style="font-size: 10px; color: #888; margin-top: 2px;">₱' . number_format($price, 2) . '</span>';
            echo '</div>';
