...
document.querySelectorAll('.size-item input[type="radio"]').forEach(radio => {
...
function handleQuickView(input) {
    let data = (input instanceof HTMLElement) ? input.dataset : input;
    
    lastCheckedSize = null;
    document.querySelectorAll('.size-item input[type="radio"]').forEach(r => r.checked = false);
    document.getElementById('zoomContainer').classList.remove('is-zoomed');
    resultsPopup.style.display = 'none';
    ...
    sizes.forEach(size => {
        const stockValue = data[size] || data['stock_' + size] || 0;
